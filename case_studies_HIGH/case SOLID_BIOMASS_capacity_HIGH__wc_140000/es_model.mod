...
##############################
#de Chambost et al. (2026) addition
##############################

set WOOD_RESOURCES within RESOURCES;

param wood_cap >= 0;   #[GWh/year] Maximum total use of solid biomass

subject to SolidBiomassCap:
    sum {r in WOOD_RESOURCES,
         t in PERIODS,
         h in HOUR_OF_PERIOD[t],
         td in TYPICAL_DAY_OF_PERIOD[t]}
        (F_t[r, h, td] * t_op[h, td])
    <= wood_cap;
